...
def run_pipeline(sources_table,
                ...
                xp_continuous_output_file='../Data/xp_continuous_downloaded.csv',
                eq_widths_output_file='../Data/Test_EqWidths',
                extrema_output_file='../Data/Test_Extrema',
                time_stamps=False,
                produce_eq_widths=True,
                config_file='../Data/test.xml'):
    ...
    config_file : str, optional
            ...
            number of cores, output format, and which lines to analyze (default: '../Data/test.xml').
    Returns
    -------
    ...
    - Writes extrema data to ../Data/<extrema_output_file>.pkl if enabled in the config.
    - Writes equivalent-width results to ../Data/<eq_widths_output_file>.<ext> when the
    ...
    user_input = dio.read_xml(config_file)
    ...
    if provide_all_extrema_flag:
        print("Saving all extrema information to file ../Data/" + extrema_output_file + ".pkl")
        with open('../Data/'+extrema_output_file+'.pkl', 'wb') as f:
    ...
    if produce_eq_widths:
        ...
        if provide_eq_widths_flag:
            print("Saving equivalent widths to file ../Data/" + eq_widths_output_file + "." + output_format)
            if output_format == 'csv':
                RESULT.to_csv('../Data/'+eq_widths_output_file+'.csv', index=False)
            
            # it will be the fastest to write to parquet
            # elif output_format == 'parquet':
            #     RESULT.to_parquet('../Data/'+eq_widths_output_file+'.parquet', index=False)
